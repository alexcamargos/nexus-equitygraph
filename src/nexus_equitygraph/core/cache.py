...
class CacheManager:
    ...
    def is_cache_valid(self, file_path: Path, expiry_duration: timedelta = timedelta(days=1)) -> bool:
        """Check if the cache file is still valid based on the expiry duration.
...
class JSONCacheManager(CacheManager):
    ...
    def load_cache(
        self, sub_directory: Path | str, file_name: str, expiry_duration: timedelta = timedelta(days=1)
        ...
        try:
            ...
            logger.error(f"JSON decoding error when loading cache from {file_path}: {json_error}")
        except OSError as os_error:
            logger.error(f"Error loading cache from {file_path}: {os_error}")

        return None

    def save_cache(self, sub_directory: Path | str, file_name: str, data: Dict[str, Any]) -> None:
        """Save data to a JSON cache file.
        ...
        try:
            ...
            logger.error(f"JSON encoding error (serialization) when saving cache to {file_path}: {type_error}")
        except OSError as os_error:
            logger.error(f"Error saving cache to {file_path}: {os_error}")


@lru_cache(maxsize=1)
def get_json_cache_manager(base_directory: Path = Cfg.DATA_DIRECTORY) -> JSONCacheManager:
...
__all__ = ["get_json_cache_manager"]
